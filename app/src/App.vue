<template>
  ...
  <div class="dropdown" v-if="loaded[`bar1`]">
    <DropOption id="DropOption" v-for="(value, key) in dropOptions[`bar1`][`X`]" v-show="value" :option="key"/>
    <DropOption id="DropOption" v-for="(value, key) in dropOptions[`bar1`][`Y`]" v-show="value || openedOptions[`bar1`][`Y`]" :option="key" @click="clickOption(key, value, `bar1`, `Y`)"/>
  </div>
  <div style="width: 50%;">
    <BarChart 
      v-if="loaded[`bar1`]" 
...
function clickOption (key, value, chart, plane) {
  ...
  if (dropOptions.value[chart][plane][key] == false) {
    ...
    console.log(plane)
    if (plane == "Y") {
      setChart(dataset, "N/A", key, "bar1")
    }

    openedOptions.value[chart][plane] = false //Closing the dropdown
  } else {
    openedOptions.value[chart][plane] = true
  }
}
